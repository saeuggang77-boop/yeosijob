...
export async function PATCH(
  ...
  try {
    ...
    const review = await prisma.partnerReview.findUnique({ where: { id } });
    if (!review) {
    ...
    console.error("Admin partner review update error:", error);
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.partnerReview.delete({ where: { id } });

    return NextResponse.json({ message: "리뷰가 삭제되었습니다" });
  } catch (error) {
    console.error("Admin partner review delete error:", error);
    return NextResponse.json({ error: "서버 오류가 발생했습니다" }, { status: 500 });
  }
}
