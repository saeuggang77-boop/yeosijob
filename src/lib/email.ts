...
export async function sendVerificationEmail(email: string, token: string) {
  ...
  const verifyLink = `${process.env.AUTH_URL}/verify-email?token=${token}`;
  ...
  await resend.emails.send({
    ...
    html: `
      <div style="max-width: 480px; margin: 0 auto; font-family: sans-serif;">
        ...
        <p>여시잡 회원가입을 완료하려면 아래 버튼을 클릭해주세요.</p>
        <a href="${verifyLink}" style="display: inline-block; padding: 12px 24px; background: #c8a84e; color: #fff; text-decoration: none; border-radius: 6px; font-weight: bold;">
          이메일 인증하기
        </a>
